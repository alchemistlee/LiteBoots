...
_ENFR_TRAIN_LARGE_DATA = [
    [
        ...
        ("commoncrawl.fr-en.fr", "commoncrawl.fr-en.en")
    ],
    [
        "http://www.statmt.org/wmt13/training-parallel-europarl-v7.tgz",
        ("training/europarl-v7.fr-en.fr", "training/europarl-v7.fr-en.en")
    ],
    [
    ...
    [
        ...
        ("un/undoc.2000.fr-en.fr", "un/undoc.2000.fr-en.en")
    ],
    [
        "https://s3.amazonaws.com/web-language-models/paracrawl/release1/enfr.tgz", #hubo data
        ("enfr/enfr.fr",
         "enfr/enfr.en")
    ],
]
_ENFR_TEST_LARGE_DATA = [
    [
        ...
        ("dev/newstest2013.fr", "dev/newstest2013.en")
    ],
]


@registry.register_problem
class TranslateFrenWmtSmall8k(translate.TranslateProblem):
...
@registry.register_problem
class TranslateFrenWmtSmall32k(TranslateFrenWmtSmall8k):
...
@registry.register_problem
class TranslateFrenWmt8k(TranslateFrenWmtSmall8k):
...
@registry.register_problem
class TranslateFrenWmt32k(TranslateFrenWmtSmall32k):
...
@registry.register_problem
class TranslateFrenWmt32kPacked(TranslateFrenWmt32k):
  ...
  def use_vocab_from_other_problem(self):
    return TranslateFrenWmt32k()


@registry.register_problem
class TranslateFrenWmt32kWithBacktranslateFr(TranslateFrenWmt32k):
...
@registry.register_problem
class TranslateFrenWmt32kWithBacktranslateEn(
    TranslateFrenWmt32kWithBacktranslateFr):
  ...
  def backtranslate_data_filenames(self):
    """List of pairs of files with matched back-translated data."""
    # Files must be placed in tmp_dir, each similar size to authentic data.
    return [("fr_mono_fr.txt%d" % i, "fr_mono_en.txt%d" % i) for i in [0, 1, 2]]


@registry.register_problem
class TranslateFrenWmtSmallCharacters(translate.TranslateProblem):
  """Problem spec for WMT En-Fr translation."""
...
@registry.register_problem
class TranslateFrenWmtCharacters(TranslateFrenWmtSmallCharacters):
...
@registry.register_problem
class TranslateFrenWmtMulti64k(TranslateFrenWmtSmall32k):
...
@registry.register_problem
class TranslateFrenWmtMulti64kPacked1k(TranslateFrenWmtMulti64k):
  """Translation with muli-lingual vocabulary."""
